...
func TestCacheReturnsCopyWithDecrementedTTL(t *testing.T) {
	cache := New(Config{MaxEntries: 1, MaxTTL: time.Minute, CleanupInterval: time.Hour})
	defer cache.Close()
	...
	cache.Set(question, message)

	message.Answer[0].Header().Ttl = 1
	cached := cache.Get(dns.Question{Name: "example.com.", Qtype: dns.TypeA, Qclass: dns.ClassINET})
	if cached == nil {
	...
	if ttl := cache.Get(question).Answer[0].Header().Ttl; ttl == 0 {
		t.Fatal("cache returned a shared message")
	}
}

func TestCacheDoesNotStoreTTLsBelowMinimum(t *testing.T) {
	cache := New(Config{MaxEntries: 1, MinTTL: time.Minute, MaxTTL: time.Hour, CleanupInterval: time.Hour})
	defer cache.Close()
	...
	cache.Set(question, message)
	if cached := cache.Get(question); cached != nil {
